...
class SettingsProvider extends ChangeNotifier {
  ...
  /// Re-read the provider-related prefs (provider configs, ordering, grouping,
  /// pinned models, current model selection) without re-running the one-shot
  /// migrations, request-log toggles, haptics side-effects, or display/theme
  /// settings that [Future._load] performs.
  ///
  /// Used after a backup restore has rewritten `provider_configs_v1` and
  /// related keys on disk: the in-memory map otherwise keeps the pre-restore
  /// snapshot, so the UI keeps showing the old provider list until the user
  /// force-kills the app. Paired with
  /// [refreshProvidersAfterRestore] from the same change-set so every
  /// restore entry point (backup_page, recovery_page) reflects imported
  /// providers without a restart.
  Future<void> reloadFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();

    // providers order
    _providersOrder = prefs.getStringList(_providersOrderKey) ?? const [];
    ...
    if (cfgStr != null && cfgStr.isNotEmpty) {
      try {
        ...
        _providerConfigs = raw.map(
          (k, v) => MapEntry(
            k,
            ProviderConfig.fromJson(v as Map<String, dynamic>),
          ),
        );
      } catch (e, st) {
        assert(() {
          debugPrint(
            '[SettingsProvider] reloadFromPrefs providerConfigs decode failed: $e',
          );
          debugPrint('$st');
          return true;
        }());
    ...
    // provider grouping (groups definitions + provider->group map + collapse)
    try {
      final groupsStr = prefs.getString(_providerGroupsKey) ?? '';
      _providerGroups = groupsStr.isEmpty
          ? const <ProviderGroup>[]
    ...
    // selected model — respect the defaultModelSeeded sentinel so reloads
    // don't re-seed DeepSeek over a user-cleared selection, but DO honor a
    // freshly-imported selection when one appears on disk (merge mode keeps
    // an existing user selection, overwrite mode replaces it).
    final sel = prefs.getString(_selectedModelKey);
    if (sel != null && sel.contains('::')) {
      final parts = sel.split('::');
      if (parts.length >= 2) {
    ...
    } else if (!(prefs.getBool(_defaultModelSeededKey) ?? false)) {
      _currentModelProvider = 'DeepSeek';
      _currentModelId = 'deepseek-v4-flash';
      await prefs.setBool(_defaultModelSeededKey, true);
      await prefs.setString(
